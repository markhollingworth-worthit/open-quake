...
1. Read `AGENTS.md` first and follow its drop-in app rules.
2. Inspect a nearby existing app, usually `apps/teams-deck` or `apps/test-app`, before creating patterns.
...
QNAP direct browser fetch can fail on TLS or CORS. If live LAN APIs must work from a drop-in app, use a generic host capability or app-local `server.js` rather than a QNAP-specific host route.
